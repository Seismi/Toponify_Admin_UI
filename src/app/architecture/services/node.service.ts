...
} from '../store/models/node-link.model';
...
export class NodeService {
  ...
  getNodes(
    queryParams?: GetNodesRequestQueryParams
  ): Observable<NodesApiResponse> {
    const params = queryParams
      ? this.toHttpParams(queryParams)
      : new HttpParams();
  ...
  getNodeLinks(
    queryParams?: GetLinksRequestQueryParams
  ): Observable<NodeLinksApiResponse> {
    const params = queryParams
      ? this.toHttpParams(queryParams)
      : new HttpParams();
  ...
  getNodeUsageView(
    id: string,
    queryParams?: { workPackageQuery: string[] }
  ): Observable<any> {
    const params = queryParams
      ? this.toHttpParams(queryParams)
      : new HttpParams();
  ...
  updateLayoutNodesLocation(
    layoutId: string,
    data: { id: string; locationCoordinates: string }
  ): Observable<any> {
    return this.http.put<any>(
      `/layouts/${layoutId}/nodes/location`,
      { data: data },
      httpOptions
    );
  }
  // FIXME: define missing types
  updateLayoutNodeLinksRoute(
    layoutId: string,
    data: { id: string; points: any[] }
  ): Observable<any> {
    return this.http.put<any>(
      `/layouts/${layoutId}/nodelinks/route`,
      { data: data },
      httpOptions
    );
  }
