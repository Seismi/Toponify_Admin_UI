...
export function updateShapeShadows(): void {
  ...
  shapesToCorrect.forEach(function(figure: string) {
    ...
    const definitionCopy = (figureDefinition as (
      a: go.Shape,
      b: number,
      c: number
    ) => go.Geometry).bind(null);
...
export class CustomLinkShift extends LinkShiftingTool {
  ...
  public doMouseUp(): void {
    const shiftedLink = (this['_handle'].part as go.Adornment).adornedObject
      .part;
...
export class CustomLink extends go.Link {
  ...
  public computePoints(): boolean {
    ...
    const linkShiftingTool = toolManager.mouseDownTools
      .toArray()
      .find(function(tool) {
      ...
      $('ContextMenuButton', $(go.TextBlock, 'show / hide RADIO alert', {}), {
        click: function(event, object) {
          ...
          event.diagram.model.setDataProperty(
            modelData,
            'showRadioAlerts',
            !modelData.showRadioAlerts
          );

          // Redo layout for node usage view after updating RADIO alert display setting
          if (
            thisService.filterService.getFilter().filterLevel === Level.usage
          ) {
  ...
  getPartContextMenu(): go.Adornment {
    ...
    return $(
      ...
      $(
        ...
        new go.Binding('visible', '', function(object, event) {
          if (event.diagram.findNodeForData(object) !== null) {
            ...
            return (
              object.category === linkCategories.data &&
      ...
      $(
        ...
        {
          click: function(event, object) {
            diagramLevelService.displayUsageView(
              event,
              (object.part as go.Adornment).adornedObject
            );
          }
        },
        new go.Binding('visible', '', function(object, event) {
