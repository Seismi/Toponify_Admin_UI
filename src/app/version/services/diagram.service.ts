...
import { AddDimensionLink, UpdateDimensionLink } from '@app/version/store/actions/dimension-link.actions';
import { AddDimension, UpdateDimension } from '@app/version/store/actions/dimension.actions';
import { AddModelLink, UpdateModelLink } from '@app/version/store/actions/model-link.actions';
import { AddModel, UpdateModel } from '@app/version/store/actions/model.actions';
import { AddSystemLinks, UpdateSystemLinks } from '@app/version/store/actions/system-link.actions';
import { AddVersionSystem, UpdateVersionSystem } from '@app/version/store/actions/version-system.actions';
...
import { SetMapView } from '../store/actions/mapview.actions';
import { System, systemCategories } from '../store/models/system.model';
import { VersionState } from '../store/reducers';
...
const addNodeActionMapping = {
  [Level.system]: AddVersionSystem,
  [Level.model]: AddModel,
  [Level.dimension]: AddDimension,
  [Level.element]: {/* Placeholder */}
};

const addLinkActionMapping = {
  [Level.system]: AddSystemLinks,
  [Level.model]: AddModelLink,
  [Level.dimension]: AddDimensionLink,
  [Level.element]: {/* Placeholder */}
};

const updateNodeActionMapping = {
  [Level.system]: UpdateVersionSystem,
  [Level.model]: UpdateModel,
  [Level.dimension]: UpdateDimension,
  [Level.element]: {/* Placeholder */}
};

const updateLinkActionMapping = {
  [Level.system]: UpdateSystemLinks,
  [Level.model]: UpdateModelLink,
  [Level.dimension]: UpdateDimensionLink,
  [Level.element]: {/* Placeholder */}
};
...
export class DiagramService implements OnDestroy {
  ...
  constructor(
    private store: Store<VersionState>,
  ...
  changeLevel(diagram: go.Diagram, level: Level): void {
    ...
    const paletteViewLinks = [{
      ...
      targetCardinality: 1,
      route: [0, 0, 80, -80],
      linkDetailAvailable: false,
      isTemporary: true
    }];

    // Variables to contain name of the property to use for link sources and targets
    let linkSourcePropName: string;
    let linkTargetPropName: string;
    ...
    if (level === Level.system) {
      ...
      if (!this.mapView) {
        paletteViewNodes = [new Dimension({ id: 'New Dimension', name: 'New Dimension' })];
      }

      linkSourcePropName = 'sourceDimension';
      linkTargetPropName = 'targetDimension';
    } else if (level === Level.element) {
      paletteViewNodes = [
        new Element({ id: 'New List Element',
          name: 'New List Element',
          category: elementCategories.mdElement,
          subcategory: elementSubcategories.mdList }),
        new Element({ id: 'New Structural Element',
          name: 'New Structural Element',
          category: elementCategories.mdElement,
          subcategory: elementSubcategories.mdStructure }),
        new Element({ id: 'New Rollup Element',
          name: 'New Rollup Element',
          category: elementCategories.mdElement,
          subcategory: elementSubcategories.mdRollup
        }),
        new Element({ id: 'New Or Rule',
          name: 'New Or Rule',
          category: elementCategories.mdRule,
          subcategory: elementSubcategories.or
        }),
        new Element({ id: 'New Combination Rule',
          name: 'New Combination Rule',
          category: elementCategories.mdRule,
          subcategory: elementSubcategories.combination
        }),
        new Element({ id: 'New Inclusion Rule',
          name: 'New Inclusion Rule',
          category: elementCategories.mdRule,
          subcategory: elementSubcategories.inclusion
        }),
        new Element({ id: 'New Exclusion Rule',
          name: 'New Exclusion Rule',
          category: elementCategories.mdRule,
          subcategory: elementSubcategories.exclusion
        }),
        new Element({ id: 'New Custom Rule',
          name: 'New Custom Rule',
          category: elementCategories.mdRule,
          subcategory: elementSubcategories.custom
        })
      ];

      linkSourcePropName = 'sourceElement';
      linkTargetPropName = 'targetElement';
    }
    ...
    diagram.model = $(go.GraphLinksModel, {
      nodeKeyProperty: 'id',
      linkKeyProperty: 'id',
      linkFromKeyProperty: linkSourcePropName,
      linkToKeyProperty: linkTargetPropName,
  ...
  displayMapView(event: any, object: go.Link): void {
    ...
    if (!object.data.linkDetailAvailable) {
      return;
    }
  ...
  createObjects(event: any): void {
    ...
    event.subject.each(function(part: go.Part) {
      ...
      if (part instanceof go.Node) {
        ...
        sendData.location = [{view: 'Default', locationCoordinates: part.data.location}];
  ...
  updatePosition(event: any): void {
    ...
    if (links.length > 0) {
      this.store.dispatch( new updateLinkActionMapping[currentLevel](links) );
    }
  }
  ...
  updateLinkConnections(event: any): void {
    ...
    if (link.fromNode && link.toNode) {
      ...
      if (link.data.isTemporary) {
        ...
        sendData.route = this.standardDisplay ? [{view: 'Default', points: link.data.route}] : [];
  ...
  getLinkMasterDataTemplate(forPalette: boolean = false) {
    return $(
      ...
      // The 'to' arrowhead
      $(go.Shape,
        // Display arrow when target is a master data rule
        new go.Binding('toArrow',
          'toNode',
          function(element) {
            if (element) {
              if (element.category === elementCategories.mdRule) {
                return 'Triangle';
              }
            }
            return '';
