...
Given('the "Topology" "Canvas" Tab is selected', function(screen, tab) {
  cy.get('smi-switch-view-tabs')
    .find(`[aria-posinset=${pages['tabs']['Canvas']}]`)
    .click();
});

Given('the "Topology" "Systems" Tab is selected', function(screen, tab) {
  cy.get(`[aria-posinset=${pages['tabs']['Systems']}]`).click();
});

Given('the "Topology" "Interfaces" Tab is selected', function(screen, tab) {
  cy.get(`[aria-posinset=${pages['tabs']['Interfaces']}]`) // use the current pane set up file to identify the posinset for angular materials tab as data-qa cannot be used
    .click();
});
